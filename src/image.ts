...
function renderGenerate(value: GenerateResult): ContentBlock[] {
  return [
    ...
    ...value.references.map(item => ({ type: 'image' as const, attachment: item.attachment as never })),
    ...value.images.map(item => ({ type: 'image' as const, attachment: item.attachment as never })),
  ]
}
...
function renderList(value: ListResult): ContentBlock[] {
  return [
    ...
    ...value.items.map(item => ({ type: 'image' as const, attachment: item.attachment as never })),
  ]
}
