/**
 * Shared Host-only media admission for image editing and video analysis.
 *
 * Implements strict path containment, TOCTOU race mitigation, magic bytes detection
 * (PNG, JPEG, WebP, GIF, AVIF), and session history traversal.
 */
...
export const DEFAULT_VIDEO_BYTES = 33554432; // 32MB
export const DEFAULT_INLINE_IMAGE_BYTES = 20971520; // 20MB
export const IMAGE_HANDLE_PATTERN = /^image:[^\s\p{C}]{1,256}$/u;

export type ExtendedImageMediaType = ImageMediaType | "image/avif";
...
export async function admitImageBytes(
  ...
  if (mediaType === undefined || !limits.mediaTypes.includes(mediaType as ImageMediaType)) {
...
/**
 * Decode and admit base64 image data.
 */
...
/**
 * Resolve an explicit image handle authorized in session history.
 */
export async function admitSessionImage(
  ...
  throwIfAborted(signal);
  if (!IMAGE_HANDLE_PATTERN.test(handle)) {
    throw new MediaAdmissionError("The image handle is invalid", "MEDIA_HANDLE_INVALID");
  }

  const ref = authorizedSessionImages(agent).get(handle);
  if (ref === undefined) {
    throw new MediaAdmissionError("The image handle is not authorized by this session", "MEDIA_HANDLE_UNAUTHORIZED");
  }
...
/**
 * Resolve an image from workspace with containment & TOCTOU checks.
 */
...
/**
 * Admit an MP4 container from workspace.
 */
...
/**
 * Read workspace file bytes with two-phase TOCTOU race detection and symlink exclusion.
 */
export async function readStableWorkspaceBytes(
  fs: Pick<FileSystem, "resolve" | "contains" | "readBytes" | "lstat" | "stat">,
  workspace: string,
  path: string,
  maxBytes: number,
  kind: MediaKind,
  ...
  const initialPath = await fs.lstat(path, { cwd: workspace, ...signal ? { signal } : {} });
  if (initialPath?.type === "symlink") {
    throw new MediaAdmissionError(`The workspace ${kind} is a symbolic link`, symlinkCode);
  }
  if (initialPath === undefined || initialPath.type !== "file") {
    throw new MediaAdmissionError(`The workspace ${kind} is not a regular file`, regularCode);
  }
  ...
  if (!fs.contains(root, target)) {
    throw new MediaAdmissionError(`The workspace ${kind} is outside the active workspace`, containmentCode);
  }
  ...
  if (before.version !== initialPath.version) {
    throw new MediaAdmissionError(`The workspace ${kind} changed before it could be read`, changedCode);
  }

  const data = await fs.readBytes(target, signal, maxBytes);

  const finalPath = await fs.lstat(path, { cwd: workspace, ...signal ? { signal } : {} });
...
/**
 * Detect image media type from magic bytes, supporting PNG, JPEG, WebP, GIF, and AVIF.
 */
export function detectImageMediaType(data: Uint8Array): ExtendedImageMediaType | undefined {
  ...
  // AVIF: ISOBMFF ftyp avif / avis
  if (data.length >= 12 && ascii(data, 4, 8) === "ftyp") {
    const majorBrand = ascii(data, 8, 12);
    if (majorBrand === "avif" || majorBrand === "avis") return "image/avif";

    // Scan compatible brands in first 64 bytes
    const ftypSize =
      ((data[0] ?? 0) << 24) |
      ((data[1] ?? 0) << 16) |
      ((data[2] ?? 0) << 8) |
      (data[3] ?? 0);
    const limit = Math.min(data.length, Math.min(ftypSize > 0 ? ftypSize : 64, 64));
    for (let i = 16; i + 4 <= limit; i += 4) {
      const brand = ascii(data, i, i + 4);
...
/**
 * Validate MP4 container magic bytes.
 */
...
/**
 * Traverse session history events to build an authorized image catalog.
 */
export function sessionImageCatalog(agent: Agent): readonly SessionImageCatalogEntry[] {
  ...
  const visit = (value: unknown, origin: SessionImageCatalogEntry["origin"], depth = 0) => {
    if (
      typeof value !== "object" ||
      value === null ||
      depth > 64 ||
    ...
    if (value.type === "tool-result" && Array.isArray(value.content)) {
      visit(value.content, origin, depth + 1);
    }
  };

  const rawEvents = (agent as unknown as { session?: { snapshotEvents?(): Iterable<unknown> } }).session?.snapshotEvents?.();
  if (rawEvents) {
    for (const rawEvent of rawEvents) {
      if (!isRecord(rawEvent) || !isRecord(rawEvent.data)) continue;
      if (rawEvent.type === "user/message") {
        visit(rawEvent.data.content, "user");
      } else if (rawEvent.type === "assistant/message" && isRecord(rawEvent.data.message)) {
        visit(rawEvent.data.message.content, "reference");
      } else if (rawEvent.type === "tool/result" && isRecord(rawEvent.data.message)) {
        visit(rawEvent.data.message.content, "generated");
      }
    }
  }
...
function parseImageRef(value: unknown): ImageAttachmentRef | undefined {
  if (
    ...
    !isPositiveSafeInteger(value.bytes, 1073741824) ||
    !isPositiveSafeInteger(value.width, 1000000) ||
    !isPositiveSafeInteger(value.height, 1000000)
  ) {
    return undefined;
  }
...
function safeName(value: string): string {
  const base = [...basename(value)]
    .map((c) => (c.charCodeAt(0) < 32 || c.charCodeAt(0) === 127 || c === "/" || c === "\\" ? "_" : c))
    .join("")
    .slice(0, 256);
...
function fsResolveOptions(cwd: string, signal?: AbortSignal): { cwd: string; signal?: AbortSignal } {
...
function positive(value: number | undefined, fallback: number): number {
  return value === undefined || !Number.isFinite(value) || value <= 0 ? fallback : Math.min(Math.floor(value), 134217728);
}

function throwIfAborted(signal?: AbortSignal): void {
  if (signal?.aborted) {
    throw new MediaAdmissionError("Media admission was cancelled", "MEDIA_CANCELLED");
  }
}
